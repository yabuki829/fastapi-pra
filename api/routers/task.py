...
async def list_tasks(db: AsyncSession = Depends(get_db)):
    task_list = await task_crud.get_task_with_done(db)
    return task_list
...
async def update_task(task_id: int, task_body: task_schema.TaskCreate, db: AsyncSession = Depends(get_db)):
    ...
    edit_task = await task_crud.update_task(db, task_body, original=task)
    return edit_task
...
async def delete_task(task_id:int,db: AsyncSession = Depends(get_db)):
    ...
    if task is None:
        raise HTTPException(status_code=404, detail="Task not found")
    del_task = await task_crud.delete_task(db,original=task)
    return del_task
